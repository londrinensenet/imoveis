...
- **Worker/painel:** cookie `Secure`, `HttpOnly`, `SameSite=Strict`, ID token Google/sessão assinada, adulteração, expiração, logout, autenticação, papéis, isolamento, acesso cruzado, sincronização geral/individual, CSRF/origem, CORS com credenciais, HTTPS, `no-store`, payload declarado e transmitido sem tamanho, validação criptográfica de tokens Google e rejeição de contas não autorizadas.
